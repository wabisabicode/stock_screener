...
class StockData(db.Model):
    ...
    inv_to_rev_mrq = db.Column(db.Float)  # Inventory to revenue (most recent quarter)
    ...
    mrq_gp_margin = db.Column(db.Float)  # Gross profit margin (most recent quarter)
    av_gp_margin = db.Column(db.Float)  # Average gross profit margin
    mrq_ocf_margin = db.Column(db.Float)  # Operating cash flow margin (most recent quarter)
    av_ocf_margin = db.Column(db.Float)  # Average operating cash flow margin
    mrq_fcf_margin = db.Column(db.Float)  # Free cash flow margin (most recent quarter)
    ...
    def update_or_create(session, stock_dict):
        ...
        if not symbol:
            raise ValueError("The stock dictionary must contain the 'symbol' key.")
        ...
        if stock:
            ...
            print(f"Stock with symbol {symbol} not found. Adding a new record.")
            new_stock = StockData(**stock_dict)
            session.add(new_stock)
            session.commit()
            print(f"New stock {symbol} added successfully.")
